...
class DrugOOD(InMemoryDataset):
    ...
    def raw_file_names(self):
        return ['lbap_general_ic50_scaffold.json']
    ...
    def download(self):
        if not osp.exists(osp.join(self.raw_dir, 'lbap_general_ic50_scaffold.json')):
            print(self.raw_dir)
    ...
    def process(self):
        ...
        for idx, case in enumerate(raw_dataset[:num]):
            ...
            case['smiles'].y = torch.tensor(case['reg_label'], dtype=torch.long).unsqueeze(dim=0)
            case['smiles'].idx = idx
            data = case['smiles']
