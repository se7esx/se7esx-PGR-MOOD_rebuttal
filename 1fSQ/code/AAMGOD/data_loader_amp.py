...
def get_ood_dataset(args, train_per=0.8, need_str_enc=True):
    ...
    DrugooD = not DS.startswith('DrugOOD')
    path = osp.join(osp.dirname(osp.realpath(__file__)), '.', 'data', DS)
    path_ood = osp.join(osp.dirname(osp.realpath(__file__)), '.', 'data', DS_ood)
    '''
    from register import register
    register.datasets ={"GOODPCBA": GOODPCBA(root=osp.join(r"/home/wangyili/shenxu/G-OOD-D-main/data/", 'pcba/'),
                                                          domain="scaffold",
                                                          shift="covariate")}
    
    datasets, meta_info = register.datasets["GOODPCBA"].load(dataset_root=osp.join(r"/home/wangyili/shenxu/G-OOD-D-main/data/", 'pcba/'),
                                                          domain="scaffold",
                                                          shift="covariate",
                                                         )
    ...
    data_list_train = DataLoader(data_list_train, batch_size=args.batch_size, shuffle=True)
    data_list_valid = DataLoader(data_list_valid, batch_size=args.batch_size, shuffle=True)
    data_list_test = DataLoader(data_list_test, batch_size=args.batch_size, shuffle=True)
    data_list_all = DataLoader(data_list_all, batch_size=args.batch_size, shuffle=True)
    data_list_nontrain = DataLoader(data_list_nontrain, batch_size=args.batch_size, shuffle=True)
