...
def run(args, model, dataloader, dataloader_valid, dataloader_eval):
    ...
    for epoch in range(1, 30):
        ...
        for data in dataloader:
            ...
            '''LGA_learner.eval()
            edge_logits = LGA_learner(emb_train[i], batch.edge_index)'''
            amp_learner.train()
        ...
        with torch.no_grad():    
            ...
            test_save = emb_train_amp.cpu().detach().numpy()
            train_save = emb_test_amp.cpu().detach().numpy()
            ood_save = emb_ood_amp.cpu().detach().numpy()
            print('saving-------------------')
...
if __name__ == '__main__':
    ...
    auroc, fpr95, aupr = metric(ood_labels, score)
    print('Original Embedding AUC:{:.3f}, FPR95:{:.3f}, AUPR:{:.3f}'.format(float(auroc), float(fpr95), float(aupr)))
