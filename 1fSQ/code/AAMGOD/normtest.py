...
def run(args, model, dataloader, dataloader_valid, dataloader_eval):
    ...
    for epoch in range(1, 300):
        ...
        for data in dataloader:
            ...
            # batch_aug_edge_weight = 0.5 + torch.sigmoid(edge_logits).squeeze()
            ...
            bal1 = args.bal1
            bal2 = args.bal1
            amp_loss = torch.mean(lof(x_amp, k=args.lofk)) + bal1 / torch.mean(lof(x_ood, k=args.lofk)) + bal2 * torch.norm(ood_edge_weight, p=2)
        ...
        with torch.no_grad():    
            ...
            for data in dataloader_eval:    
                ...
                # emb_test_amp.append(x_amp_test.cpu().numpy())
            ...
            score = lof(emb_test_amp, k=args.lofk)
            score_valid = lof(emb_valid_amp, k=args.lofk)
...
if __name__ == '__main__':
    ...
    setup_seed(args.seed)
    print(args)
    #with open('result/' + args.DS_pair + ".txt", 'a+') as f:
        #f.write('{},{},{},{}\n'.format(args.amplr, args.bal1, args.bal2, args.ratio))
        #f.write('\n')
    accuracies = {'val':[], 'test':[]}
    epochs = 30
    log_interval = 10
    ...
    lr = 0.1
    DS = args.DS
    data_train, data_val, data_test, data_all, data_nontrain, dataset_num_features = get_ood_dataset(args)
    ...
    acc_val, acc = evaluate_embedding(emb_train, y_train, emb, y, args=args)
    ...
    score = lof(torch.from_numpy(emb).to(device), k=args.lofk)
